...
MDIR=$WORKDIR/msent-$SL$TL-BPE$NBPE-$N-$WORKSPACE-test
mkdir -p $MDIR
mkdir -p $MDIR/spm
...
function train {
	...
	$MARIAN_TRAIN \
        --model $MDIR/models/model.npz --type s2s \
        --train-sets $MDIR/spm/train.spm.$SL $MDIR/spm/train.spm.$TL \
        --max-length 1000 \
        --vocabs $MDIR/spm/vocab.$SL$TL.yml $MDIR/spm/vocab.$SL$TL.yml \
        -w $WORKSPACE --mini-batch-words 1000 \
        --valid-freq 400 --save-freq 5000 --disp-freq 500 \
        --valid-metrics ce-mean-words perplexity translation bleu \
	...
	--log $MDIR/train.log --valid-log $MDIR/valid.log \
        --enc-depth 1 --dec-depth 1 --enc-cell-depth 2 \
        --tied-embeddings-all --label-smoothing 0.2 \
