...
def is_hij_zij_het(tag_info):
    if tag_info["feats"] is not None:
        try:
            return (tag_info["lemma"] == "hij" or tag_info["lemma"] == "zij" or tag_info["lemma"] == "het") and \
                    tag_info["feats"]["Person"] == "3" and \
...
if __name__ == '__main__':
    ...
    TESTDIR = "/data/s3212262/thesis_data/OpenSubtitles5/test"
    # TESTDIR = "./data/TED"
    CONTEXT_SENTS = 35
    SENTENCE_LENGTH = 150
    MD = MosesDetokenizer(lang="nl")

    testdirs = [os.path.join(directory, filedir)
                for directory in os.listdir(TESTDIR) if directory.isnumeric()
                for filedir in os.listdir(os.path.join(TESTDIR, directory))]
    you_block_num = 1
    it_block_num = 1
    ...
    for testdir in testdirs:
        ...
        align = get_align(os.path.join(TESTDIR, testdir, "en-nl.align"))
        ...
        for i in range(CONTEXT_SENTS, len(en_tok)):
            for j in range(len(en_tok[i])):
                ...
                if is_it(en_tag):
                    ...
                    if aligned_tag and is_hij_zij_het(aligned_tag) and is_valid_sentence(en_raw[i]):
                        alts = create_alternatives(nl[i], aligned_tag, ["hij", "zij", "het"])
