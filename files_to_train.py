...
if __name__ == '__main__':
    ...
    TED_DIR = "/data/s3212262/thesis_data/TED5"
    TRAIN_DIR = os.path.join(TED_DIR, "train")
    DEV_DIR = os.path.join(TED_DIR, "dev")

    # Create out directory for each out file
    out_dir = os.path.join(TED_DIR, "train_context_%i" % CONTEXT_LENGTH)
    ...
    file_dirs = [directory for directory in os.listdir(TRAIN_DIR) if directory.isnumeric()]
    ...
    # extract subdirectories containing file per directory
    file_dirs = [directory for directory in os.listdir(DEV_DIR) if directory.isnumeric()]
