...
if __name__ == '__main__':
    ...
    SUB_DIR = "/data/s3212262/thesis_data/OpenSubtitles4"
    TRAIN_DIR = os.path.join(SUB_DIR, "train")
    DEV_DIR = os.path.join(SUB_DIR, "dev")

    # Create out directory for each out file
    out_dir = os.path.join(SUB_DIR, "train_context_%i" % CONTEXT_LENGTH)
    ...
    file_dirs = [os.path.join(year, directory) for year in os.listdir(TRAIN_DIR) if year.isnumeric()
                 for directory in os.listdir(os.path.join(TRAIN_DIR, year))]
    ...
    # extract subdirectories containing file per directory
    file_dirs = [os.path.join(year, directory) for year in os.listdir(DEV_DIR) if year.isnumeric()
                 for directory in os.listdir(os.path.join(DEV_DIR, year))]
